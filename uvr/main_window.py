...
class MainWindow(TkinterDnD.Tk if is_dnd_compatible else tk.Tk):
    ...
    def fill_filePaths_Frame(self):
        ...
        self.filePaths_musicFile_Open = ttk.Button(
            ...
            command=lambda: (
                ...
                if self.inputPaths
                and os.path.isdir(os.path.dirname(self.inputPaths[0]))
            ...
            if os.path.isfile(i):
                try:
                    (
                        ...
                        if not type(sample_path) is str
                        else self.create_sample(i, sample_path)
                    )
                    is_good = True
                except Exception as e:
    ...
    def right_click_menu_paste(self, text_box=False):
        ...
        (
            ...
            if text_box
            else self.right_click_menu_delete()
        )
    ...
    def menu_view_inputs(self):
        ...
        def close_window():
            (
                ...
                if self.thread_check(self.verification_thread)
                else None
            )
    ...
    def menu_settings(self, select_tab_2=False, select_tab_3=False):  # **
        ...
        if not is_macos and self.is_gpu_available:
            gpu_list_options = lambda: self.loop_gpu_list(
    ...
    def pop_up_save_current_settings(self):
        ...
        validation = lambda value: (
            False if re.fullmatch(REG_SAVE_INPUT, value) is None else True
        )
    ...
    def pop_up_vr_param(self, vr_model_hash):
        ...
        vr_new_toggle = lambda: (
            enable_new_vr_op() if is_new_vr_model_var.get() else disable_new_vr_op()
        )
    ...
    def pop_up_input_stem_name(
        ...
        validation = lambda value: (
            False if re.fullmatch(REG_INPUT_STEM_NAME, value) is None else True
        )
    ...
    def process_preliminary_checks(self):
        ...
        if self.chosen_process_method_var.get() == VR_ARCH_PM:
            continue_process = lambda: (
                False if self.vr_model_var.get() == CHOOSE_MODEL else True
            )
        if self.chosen_process_method_var.get() == MDX_ARCH_TYPE:
            continue_process = lambda: (
                False if self.mdx_net_model_var.get() == CHOOSE_MODEL else True
            )
        if self.chosen_process_method_var.get() == DEMUCS_ARCH_TYPE:
            continue_process = lambda: (
                False if self.demucs_model_var.get() == CHOOSE_MODEL else True
            )
